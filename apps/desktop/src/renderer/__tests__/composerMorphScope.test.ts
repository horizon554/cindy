...
const permissionPrompt = read('components/new-chat/PermissionPrompt.tsx');
...
// 2026-07-26 澄清:"composer" 按**位置**读而非按父组件读 —— pending 交互卡片顶替 composer
// 时,卡片上的权限 chip 仍是同一个控件、同一个位置,归本作用域(DESIGN.md §14.4 作用域边界)。
describe('composer morph scope', () => {
  ...
  it('权限卡片顶替 composer 时复用同一枚 PermissionSelector,不另搭下拉', () => {
    expect(permissionPrompt).toContain("import { PermissionSelector } from './PermissionSelector'");
    // 私搭 Radix 弹层会让同一位置的同一控件分叉出两套开合动效。
    expect(permissionPrompt).not.toContain('<Popover');
    expect(permissionPrompt).not.toContain('<MorphPopover');
  });
